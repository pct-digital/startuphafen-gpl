import { MergeDeepHelper } from './merge-deep';

describe('MergeDeepHelper', () => {
  let helper: MergeDeepHelper;

  beforeEach(() => {
    helper = new MergeDeepHelper();
  });

  it('can be instantiated', () => {
    expect(helper).toBeInstanceOf(MergeDeepHelper);
    expect(typeof helper.merge).toBe('function');
  });
  ...
  it('merges very simple objects', () => {
    ...
    expect(helper.merge(o1, o2)).toEqual({
      a: 42,
      b: 2,
      c: 3,
      d: 17,
  ...
  it('does a more complex example with simple arrays (no objects with id properties)', () => {
    ...
    expect(helper.merge(o1, o2, o3)).toEqual({
      a: {
        x: 17,
        z: ['QQQ'],
        Q: {
  ...
  it('merging when target array has duplicate ids throws an error', () => {
    ...
    expect(() => helper.merge(o1, o2)).toThrow(Error);
  });
