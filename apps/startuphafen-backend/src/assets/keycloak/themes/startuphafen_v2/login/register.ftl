    ...
    <#elseif section = "form">
        <form id="kc-register-form" class="${properties.kcFormClass!}" action="${url.registrationAction}" method="post" novalidate="novalidate">
            <@userProfileCommons.userProfileFormFields; callback, attribute>
                ...
                <#-- render password fields just under the username or email (if used as username) -->
                    <#if passwordRequired?? && (attribute.name == 'username' || (attribute.name == 'email' && realm.registrationEmailAsUsername))>
                        <#if msg("registerUsernameRequirements") != "">
                            <p aria-live="polite">${msg("registerUsernameRequirements")}</p>
                        </#if>
                        <@field.password name="password" required=true label=msg("password") autocomplete="new-password" />
                        <#if msg("registerPasswordRequirements") != "">
                            <p aria-live="polite">${msg("registerPasswordRequirements")}</p>
                        </#if>
            ...
            <#if captchaFoxRequired??>
                <div class="form-group">
                    <div class="${properties.kcInputWrapperClass!}">
                        <div class="captchafox" id="captchafox" data-mode="${captchaFoxMode!'inline'}" data-sitekey="${captchaFoxSiteKey}"></div>
                    </div>
                </div>
            </#if>
